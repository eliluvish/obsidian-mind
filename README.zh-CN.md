...
> **一个让 Claude Code 记住一切的 Obsidian 仓库。** 开始一次会话，聊聊你的一天，Claude 会处理剩下的一切——笔记、链接、索引、项目上下文。每一次对话都建立在上一次的基础之上。

---

## 🔴 问题

Claude Code 功能强大，但它会遗忘。每次会话都从零开始——不了解你的目标、你的项目、你的工作模式、你的成就。你不得不反复解释相同的事情。三次对话前做出的决策就丢失了。知识永远无法积累。
...
Claude: "你有 5 个活跃项目。patient-registry 有一个待处理的
         关于 Stimulus 迁移的 ADR。lab-inventory 已经 12 天没有
         活动了。sample-tracker 的部署操作手册需要更新。"
```
...
# → "你有 5 个活跃项目。patient-registry 昨天有 3 次提交。
#    lab-inventory 有一条 2 周前的过时工作笔记。"
```
...
# → 读取项目 README（Rails 7.1，部署到 research-app01，PI 是 Dr. Smith）
# → 加载最近 3 条工作笔记和未决决策
# → "你上次在处理批量 CSV 导入超时的修复。还有一个待处理的
#    决策：是否切换到 Stimulus 来做表单验证。"
```

**从 GitHub Issue 创建工作笔记：**

```bash
/issue-capture https://github.com/hospital/patient-registry/issues/47
# → 通过 gh CLI 获取 Issue
# → 创建 work/projects/patient-registry/notes/2026-04-05-bulk-import-timeout.md
# → 自动填充 frontmatter：project、github_issue、description
```
...
```
→ 创建了 work/projects/patient-registry/notes/2026-04-05-bulk-import-requirements.md
→ 创建了决策记录："使用 Active Job 处理批量导入"
→ 更新了 reference/compliance/hipaa-audit-logging.md，添加 PHI 访问要求
→ 更新了 org/people/Dr Smith.md，添加会议上下文
```
...
**文件夹按用途组织，链接按含义组织。** 一个笔记存放在一个文件夹中（它的归属），但链接到许多笔记（它的上下文）。Claude 维护这个图谱——自动将工作笔记链接到项目、决策和人员。一个没有链接的笔记就是一个 bug。

**以项目为中心的组织方式。** 你的每个 Rails 应用在 `work/projects/` 下都有自己的文件夹，包含一个 README（仓库 URL、部署目标、干系人、合规备注）、一个 `decisions/` 文件夹用于存放 ADR，以及一个 `notes/` 文件夹用于存放工作笔记。当你执行 `/context-switch` 时，Claude 会读取整个项目文件夹来重新加载上下文。
...
**会话有设计好的生命周期。** `SessionStart` 钩子自动注入你的北极星目标、活跃项目、最近变更、待办任务和完整的仓库文件列表——Claude 每次会话都带着上下文启动，而不是从空白开始。结束时，说 "wrap up"，Claude 就会运行 `/wrap-up`——验证笔记、更新索引、发现差距。`CLAUDE.md` 规范了中间的一切：文件归档位置、链接方式、何时拆分笔记、如何处理决策和部署。
...
| 钩子 | 触发时机 | 功能 |
|------|---------|------|
| 🚀 SessionStart | 启动/恢复时 | QMD 重新索引，注入北极星目标、项目、最近变更、任务、文件列表 |
| 💬 UserPromptSubmit | 每条消息 | 对内容分类（决策、成就、架构、人员、项目、部署、合规）并注入路由提示 |
...
**切换项目**：运行 `/context-switch <project>`。Claude 会读取项目 README、最近笔记、未决决策和相关参考资料。这是管理多项目最重要的命令。

**全天**：自然地交谈。提到你做的决策、正在进行的部署、合规问题。分类钩子会引导 Claude 将每条信息归档到正确位置。对于更大量的信息转储，使用 `/dump` 一次性叙述所有内容。

**收工**：说 "wrap up"，Claude 就会调用 `/wrap-up`——验证笔记、更新索引、检查链接。

**每周**：运行 `/weekly` 进行跨项目综合——北极星对齐、模式识别、未记录的成就和下周优先事项。运行 `/vault-audit` 捕获孤立笔记、断开的链接和过时的内容。
...
| `/weekly` | 每周综合——跨项目模式、北极星对齐、未记录的成就 |
| `/weekly-review` | 轻量级跨项目回顾——已交付内容、阻塞事项、更新 Index.md |
| `/context-switch` | 重新加载项目上下文——README、最近笔记、未决决策 |
| `/project-status` | 单个项目深度检查——笔记、决策、过时条目、GitHub Issues |
| `/issue-capture` | 从 GitHub Issue URL 创建工作笔记，自动填充 frontmatter |
| `/decision` | 在对应项目的 `decisions/` 文件夹中创建 ADR，自动编号 |
| `/deploy-checklist` | 调出部署操作手册，逐项检查部署前/后检查清单 |
| `/project-archive` | 将已完成的项目从 `work/projects/` 移至 `work/archive/`，更新索引 |
...
| Base | 显示内容 |
|------|---------|
| Projects | 所有项目 README，包含状态、Rails 版本、最后更新时间 |
| Work Dashboard | 按项目分组的活跃工作笔记 |
...
Claude 将会：
1. **检测** 你的仓库版本（v1–v3.3，或识别为非 obsidian-mind 仓库）
...
> [!NOTE]
> 适用于任何 Obsidian 仓库，不仅限于 obsidian-mind。对于非 obsidian-mind 仓库，Claude 会读取每个笔记并进行语义分类——将工作笔记、人员、决策和参考资料路由到正确的文件夹。
...
6. 在 `work/projects/` 中使用 Project README 模板创建你的第一个项目文件夹
7. 开始谈论工作
...
bases/                  动态数据库视图（Projects、Work Dashboard、People、Templates）

work/
  projects/
    <project-name>/     每个 Rails 应用一个文件夹
      README.md         项目上下文：仓库、部署、技术栈、干系人、合规
      decisions/        该项目的 ADR
      notes/            该项目的按日期工作笔记
  archive/<name>/       已完成/停用的项目文件夹
  Index.md              所有工作的内容地图

org/
  people/               每人一个笔记——PI、IT 联系人、协作者
  teams/                每个团队一个笔记——成员、职责范围
  People & Context.md   组织知识的内容地图

perf/
  Brag Doc.md           成就持续记录，链接到工作笔记
...
reference/
  compliance/           HIPAA、IRB 和法规合规笔记
  ops/                  部署操作手册和运维流程
  infrastructure/       网络、服务器和环境笔记
...
- **Work Note（工作笔记）** — date、description、project、github_issue、status、tags
- **Decision Record（决策记录）** — date、description、project、status（proposed/accepted/deprecated）、context
- **Thinking Note（思考笔记）** — date、description、context、tags（暂存区——提炼后删除）
- **Project README（项目 README）** — date、description、project、status、rails_version、ruby_version、stakeholders、compliance
...
| 你的项目 | `work/projects/` — 每个应用一个文件夹，包含 README |
| 你的联系人 | `org/` — 添加 PI、IT 联系人、协作者 |
| 你的合规要求 | `reference/compliance/` — HIPAA、IRB、审计要求 |
| 你的操作手册 | `reference/ops/` — 按项目的部署流程 |
| 你的工具 | `.claude/commands/` — 根据你的 GitHub 组织进行编辑 |
