...
Review the session's completed work across all projects — issues closed, PRs merged, notes written, decisions made, meetings captured — and offer to record time entries. Use the `recording-time` skill to generate administrator-friendly descriptions from the session's git activity and conversation context, then present the proposed entries for confirmation and submission.
